...
export default function UpdateCollaboratorModal(props) {
  const { modalVisible, setModalVisible, idCollaborator, loadCollaborators } = props;
  ...
  const updateCollaborator = async () => {
    const params = { name, email, idAccessLevel, document: cpf };
    try {
  ...
  return (
    <>
      <ResultModal
        title={
          responseUpdate.error || !responseUpdate
            ? 'Falha ao atualizar colaborador(a).'
            : 'Colaborador(a) atualizado(a) com sucesso!'
        }
            ...
            <ModalFooter>
              <div className="add-product-modal-footer">
                <button
                  type="submit"
                  className="secondary"
                  disabled={!idAccessLevel || !email || !cpf || !name}
                >
                  Salvar
                </button>
              </div>
